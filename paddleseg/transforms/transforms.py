...
class Compose:
    ...
    def __call__(self, im, label=None):
        ...
        if isinstance(im, str):
            im = cv2.imread(im,0).astype('float32')
...
class CoarseDropout:
    ...
    """
    def __init__(self, max_holes=8192, max_height=4, max_width=4,
                 min_holes=4096, min_height=2, min_width=2,
    ...
    def __call__(self, im,label=None):
        if random.random() < self.prob:
            ...
            im = functional.cutout(im, holes, self.fill_value)
        if label is None:
            return (im,)
        else:
            return (im,label)
